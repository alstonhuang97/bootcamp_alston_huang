...
def remove_outliers(df: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    """Return a copy of df with rows where mask is True dropped.
    Assumptions: mask comes from detect_outliers_iqr/zscore run on the
    relevant column of df (or otherwise shares df's index) - misaligned
    masks raise rather than silently dropping the wrong rows. NaNs are
    kept, since detect_outliers_iqr/zscore never flag NaN as True.
    """
    if not mask.index.equals(df.index):
        raise ValueError("mask index does not match df index")
    return df.loc[~mask].copy()


def flag_outliers(df: pd.DataFrame, column: str, mask: pd.Series, flag_column: str = None) -> pd.DataFrame:
    """Return a copy of df with a boolean outlier-flag column added.
    Assumptions: mask shares df's index (validated below); flag_column
    defaults to '{column}_outlier' - pass an explicit name if you're
    flagging the same column with more than one method/mask so the
    columns don't collide (e.g. 'price_outlier_iqr', 'price_outlier_zscore').
    """
    if not mask.index.equals(df.index):
        raise ValueError("mask index does not match df index")
    flag_column = flag_column or f"{column}_outlier"
    out = df.copy()
    out[flag_column] = mask
    return out
